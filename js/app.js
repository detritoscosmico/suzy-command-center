...
let chartCandles=[];
let calendarEvents=SuzyCore.buildDemoCalendar();
...
function calendarStatusLabel(eventTime,now){const status=SuzyCore.calendarEventStatus(eventTime,now);if(status==="LIVE")return{status,label:"AGORA",className:"orange"};if(status==="DONE")return{status,label:"ENCERRADO",className:""};if(status==="UNKNOWN")return{status,label:"INDISPONÍVEL",className:""};const minutes=Math.max(1,Math.ceil((eventTime-now.getTime())/60000));const hours=Math.floor(minutes/60);const rest=minutes%60;return{status,label:hours?`EM ${hours}H ${rest}MIN`:`EM ${rest}MIN`,className:"green"};}
function renderCalendar(){
 const now=new Date();const list=SuzyCore.filterCalendarEvents(calendarEvents,{day:$("calendarDay").value,currency:$("calendarCurrency").value,impact:$("calendarImpact").value,now});const statuses=list.map(event=>calendarStatusLabel(event.time,now));
 $("calendarTotal").textContent=list.length;$("calendarUpcoming").textContent=statuses.filter(item=>item.status==="UPCOMING").length;$("calendarHigh").textContent=list.filter(event=>event.impact==="HIGH").length;$("calendarLive").textContent=statuses.filter(item=>item.status==="LIVE").length;
 const impactMeta={HIGH:{label:"ALTO",className:"impact-high"},MEDIUM:{label:"MÉDIO",className:"impact-medium"},LOW:{label:"BAIXO",className:"impact-low"}};
 $("calendarBody").innerHTML=list.length?list.map((event,index)=>{const date=new Date(event.time);const impact=impactMeta[event.impact];const status=statuses[index];return`<tr><td>${date.toLocaleDateString("pt-BR",{weekday:"short",day:"2-digit",month:"2-digit"})}</td><td><strong>${date.toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})}</strong></td><td><span class="currency-badge">${escapeHtml(event.currency)}</span></td><td>${escapeHtml(event.title)}</td><td><span class="impact-badge ${impact.className}">${impact.label}</span></td><td><strong class="${status.className}">${status.label}</strong></td></tr>`;}).join(""):`<tr><td colspan="6" class="empty-row">Nenhum evento demo atende aos filtros escolhidos.</td></tr>`;
 $("calendarUpdated").textContent=`Cenário demo atualizado às ${now.toLocaleTimeString("pt-BR")}`;
}
function refreshCalendar(){calendarEvents=SuzyCore.buildDemoCalendar();renderCalendar();}
...
function initialize(){populateTradeAssets();populateChartAssets();fillMissionForm();renderAssets();renderStats();renderScanner();generateChartScenario();renderCalendar();updateClock();loadAssetCatalog();}
